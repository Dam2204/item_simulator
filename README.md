...
- 캐릭터 생성 API (/character) \*post
- 캐릭터 상세 조회 API (/character/:id) \*get
- 캐릭터 삭제 API (/character/:id) \*delete
- 아이템 신규 등록 API (/item) \*post
...
1. 쿠키로 토큰을 부여하는 방식의 코드만 만져보다가, 헤더로만 토큰을 처리하려고 하니 인증 미들웨어가 먹통이 되어버렸습니다. 어떻게든 헤더에 넣기는 했는데, 로그인 후 다른 API로 연결되는 방법을 결국 찾지 못했습니다. (해설 영상을 참고해보니, 인증/인가 로직에 대한 이해 부족인 것 같아 쿠키로 토큰을 처리하는 방식으로 바꾸되 더 많은 API를 구현해보는 것으로 목표를 수정했습니다. 아이템 구매 등의 도전과제들은 TypeError 문제가 해결되지 않아, 추후 다시 도전해보려고 합니다.)
